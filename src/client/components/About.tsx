...
const FEATURES = [
  { icon: Timer, title: "Temporary inboxes", desc: "Create a disposable address that self-destructs after 24, 48, or 72 hours." },
  { icon: Radio, title: "Real-time delivery", desc: "Emails appear instantly via WebSocket — no polling, no refresh." },
  { icon: Globe, title: "Multi-domain", desc: "Bring your own domains. Each one gets catch-all routing and reserved admin mailboxes." },
  { icon: Shield, title: "Admin dashboard", desc: "Manage domains, inspect temporary inboxes, and monitor permanent mailboxes." },
];
      ...
      <section>
        ...
        <ol className="mt-4 space-y-3 text-sm leading-relaxed text-zinc-400">
          <li className="flex gap-3">
            <span className="inline-grid h-6 w-6 shrink-0 place-items-center rounded-full bg-zinc-800 text-xs font-semibold text-zinc-300">1</span>
            <span><strong className="text-zinc-200">Create an inbox</strong> — pick a domain and lifetime. You get a random address and an access token stored in your browser.</span>
          </li>
          <li className="flex gap-3">
            <span className="inline-grid h-6 w-6 shrink-0 place-items-center rounded-full bg-zinc-800 text-xs font-semibold text-zinc-300">2</span>
            <span><strong className="text-zinc-200">Receive email</strong> — Cloudflare Email Routing forwards inbound mail to the Worker, which parses it, stores the body in R2, and writes metadata to D1.</span>
          </li>
          <li className="flex gap-3">
            <span className="inline-grid h-6 w-6 shrink-0 place-items-center rounded-full bg-zinc-800 text-xs font-semibold text-zinc-300">3</span>
            <span><strong className="text-zinc-200">Instant notification</strong> — a Durable Object wakes up just long enough to push the new-email event over WebSocket, then hibernates again.</span>
          </li>
          <li className="flex gap-3">
            <span className="inline-grid h-6 w-6 shrink-0 place-items-center rounded-full bg-zinc-800 text-xs font-semibold text-zinc-300">4</span>
            <span><strong className="text-zinc-200">Automatic cleanup</strong> — an hourly cron purges expired inboxes, their emails, and all R2 objects.</span>
          </li>
        </ol>
      </section>
    </main>
